...
def select_token_by_concurrent():
    ...
    for (token1, token2), times in concurrent.items():
        ...
        if 15 < freq1 < 30 and 15 < freq2 < 30:
            print(token1, token2)
...
def select_token_by_test_concurrent():
    # train_filename = "/home/wangdq/cwmt/select_10/test.en"
    # gram = 10
    # concurrent = {}
    #
    # def _add(token1, token2):
    ...
    #                 _add(tokens[index], tokens[i])
    #
    # concurrent_filename = os.path.join(os.path.dirname(train_filename), "test.concurrent.txt")
    # with open(concurrent_filename, 'w') as f:
    #     for (token1, token2), times in concurrent.items():
    #         f.write(token1 + '\t' + token2 + '\t' + str(times) + '\n')

    concurrent_filename = "/home/wangdq/cwmt/select_10/test.concurrent.txt"
    concurrent = {}
    with open(concurrent_filename) as f:
        for line in f:
            token1, token2, times = line.strip().split('\t')
    ...
    for (token1, token2), times in concurrent.items():
        ...
        #     continue
        freq1, freq2 = freq.get(token1, 0), freq.get(token2, 0)
        if 5 < freq1 < 20 and 5 < freq2 < 20:
            select.add((token1, token2) if token1 > token2 else (token2, token1))
        # elif 5 < freq1 < 20:
        #     select.add(token1)
        # elif 5 < freq2 < 20:
        #     select.add(token2)
...
def package():
    ...
    with open(src_filename) as f_src, open(tgt_filename) as f_tgt:
        for src, tgt in zip(f_src, f_tgt):
            content.append(src)
            content.append(tgt)
    with open("/home/wangdq/cwmt/select_10/train.txt", 'w') as f:
...
if __name__ == '__main__':
    select_token_by_test_concurrent()
